...
const checkShop = async () => {
  try {
    const res = await axios.get(
      `http://localhost:8080/api/shops/my-shop?shopkeeperId=${user.id}`
    );
...
const payWithRazorpay = (pkg, durationType) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen">
      ...
      <p className="mb-4 text-gray-600">
        Please register your shop before accessing subscriptions
      </p>
...
{hasActiveSubscription && (
  <div className="mb-4 p-3 bg-yellow-100 text-yellow-800 rounded">
    You already have an active subscription. Please renew instead.
  </div>
)}
  ...
  {(pkg.durationType === "MONTHLY" ||
    pkg.durationType === "BOTH") && (
    <button
  disabled={hasActiveSubscription}
  onClick={() => payWithRazorpay(pkg, "MONTHLY")}
...
>
  ...
  {(pkg.durationType === "YEARLY" ||
    pkg.durationType === "BOTH") && (
   <button
  disabled={hasActiveSubscription}
  onClick={() => payWithRazorpay(pkg, "YEARLY")}
  className={`flex-1 px-4 py-2 rounded font-semibold ${
    hasActiveSubscription
      ? "bg-gray-400 cursor-not-allowed"
...
>
  ...
  <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
    <div className="bg-white w-[400px] rounded-xl shadow-lg p-6">
      ...
      <p className="mb-2"><strong>Package:</strong> {selectedPackage?.name}</p>
      <p className="mb-2"><strong>Type:</strong> {selectedDuration}</p>
      <p className="mb-4">
